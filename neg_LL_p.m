function [f,neg_gradient] = neg_LL_p(parms,k,d,D,B,V,Y,gammaP,lambda)
...
if(parms(1) < 0.001 || min(parms(2:d+1)) < 0.01)
    f = inf;
...
f = 0.5*k*log(2*pi) + log(det(L)) + 0.5*Z'*Z+lambda*norm(theta);
...
if nargout > 1 
    ...
    %beta_par_tau21 = inv(F'*inv(A)*F) * F'*(inv(A)*C*inv(A)) * (F*inv(F'*inv(A)*F)*(F'*inv(A)*Y) - Y)
